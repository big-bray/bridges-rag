...
own evaluation step) is future work, as is community summaries (C3, below).
...
Like `--graph`, this doesn't regress the existing text-only benchmark (no concept-style questions in
it yet — that extension is future work, same as C1). A live check against Neo4j confirmed the traversal
correctly surfaces both a paper that directly mentions a linked concept and a second paper reached only
by hopping a `RELATED_TO` edge to a related concept mentioned there.
