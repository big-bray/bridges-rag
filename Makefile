...
eval:
	uv run python -m bridges_rag.eval.cli $(if $(HYBRID),--hybrid) $(if $(RERANK),--rerank) $(if $(GRAPH),--graph) $(if $(CONCEPTS),--concepts) $(if $(GRAPH)$(CONCEPTS),--year $(YEAR) --data-dir $(DATA_DIR))
